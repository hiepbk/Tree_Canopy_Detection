...
class DeformConv(nn.Module):
    ...
    def forward(self, x, offset):
        ...
        # Ensure inputs are contiguous and float32 (CUDA kernels require float32)
        # Preserve original dtype to convert back
...
class ModulatedDeformConv(nn.Module):
    ...
    def forward(self, x, offset, mask):
        ...
        if torch.isnan(x).any() or torch.isinf(x).any():
            raise ValueError("Input x contains NaN/Inf")
        ...
        offset_float = offset.contiguous().float()
        mask_float = mask.contiguous().float()
        weight_float = self.weight.float()
        bias_float = self.bias.float() if self.bias is not None else None

        if torch.isnan(x_float).any() or torch.isinf(x_float).any():
            raise ValueError("x_float contains NaN/Inf after conversion - dtype issue?")

        x_out = modulated_deform_conv(
            x_float,
            offset_float,
            mask_float,
            weight_float,
        ...
        if torch.isnan(x_out).any() or torch.isinf(x_out).any():
            raise ValueError("C++ modulated_deform_conv_forward produced NaN/Inf")
        
        # Convert back to original dtype
        x = x_out.to(original_dtype)
        if self.norm is not None:
            x = self.norm(x)
        if self.activation is not None:
            x = self.activation(x)
